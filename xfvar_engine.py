...
def _calculate_hitter(
    ...
    repl = _hitter_replacement_profile(
        pool,
        player_stats,
    )
    ...
    for category in [
        ...
        player_total = _safe_float(
            player_stats[category]
        )

        repl_total = _safe_float(
            repl[category]
        )
...
def _calculate_sp(
    ...
    repl = _pitcher_replacement_profile(
        pool,
        player_stats,
        "SP",
    )
    ...
    # SP counts only K and QS.
    # SP is never penalized for zero saves.
...
def _calculate_rp(
    ...
    # RP counts only SV and K.
    # RP is never penalized for zero quality starts.
